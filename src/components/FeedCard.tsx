...
export function FeedNav({ onProfile, onFilter, onView, filtersActive, view = 'swipe', showFilter = true, top = 70 }: NavHandlers & { view?: View; showFilter?: boolean; top?: number }) {
  const round = { width: 28, height: 28, padding: 0, border: 0, borderRadius: '50%', background: 'var(--rule)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' } as const;
  return (
    <div style={{ position: 'absolute', top: T(top), left: 20, right: 20, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 4 }}>
      <button aria-label="profile" onClick={onProfile} style={round}><Icon name="user" size={16} color="var(--gray)" /></button>
      <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)' }}><Wordmark size="xs" /></div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        {showFilter && (
          <button aria-label="filters" onClick={onFilter} style={{ ...round, position: 'relative' }}>
        ...
        <div role="radiogroup" aria-label="feed view" style={{ display: 'flex', padding: 2, borderRadius: 50, background: 'var(--rule)' }}>
          {(['swipe', 'list'] as const).map(v => (
            <button key={v} role="radio" aria-checked={view === v} aria-label={v === 'swipe' ? 'swipe view' : 'list view'} onClick={() => onView?.(v)}
              style={{ width: showFilter ? 28 : 30, height: 24, padding: 0, border: 0, borderRadius: 50, background: view === v ? 'var(--card)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
...
export function FeedCard(p: FeedCardProps) {
  ...
  const actionBtn = { position: 'relative', width: 36, height: 36, padding: 0, border: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background 150ms' } as const;
  return (
    <article aria-label={s.title} style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', right: 14, bottom: B(124), display: 'flex', flexDirection: 'column', gap: 16, zIndex: 4 }}>
        <button aria-label={p.liked ? 'unlike' : 'like'} aria-pressed={!!p.liked} onClick={p.onLike} style={{ ...actionBtn, background: p.liked ? 'var(--alert-tint)' : 'transparent' }}>
        ...
        <button aria-label="share" onClick={p.onShare} style={{ ...actionBtn, background: 'transparent' }}>
        ...
        <button aria-label={p.saved ? 'remove bookmark' : 'bookmark'} aria-pressed={!!p.saved} onClick={p.onSave} style={{ ...actionBtn, background: p.saved ? 'var(--signal-tint)' : 'transparent' }}>
          {p.saved
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: B(58), display: 'flex', justifyContent: 'center' }}>
        <button onClick={p.onRead} className="link-btn" style={{ padding: '4px 8px', font: '600 14px/1 var(--font)', color: 'var(--signal)' }}>read full article →</button>
      </div>
